...
public class ExportItemListService extends FileExportExcelService implements AutoCloseable{
	 ...
	 * @throws DaoException
	 */
	public HSSFWorkbook getExportItemList(SearchItemDTO searchItemDTO, HSSFWorkbook workBook) throws DaoException {
		...
		for (ResultItemSearchDTO dto: mstItemList) {
			...
			//旧品番
			callCreateCell(row, colIdx++).setCellValue(castRichTextString(dto.getOldItemCode()));

			//商品名
			callCreateCell(row, colIdx++).setCellValue(castRichTextString(dto.getItemNm()));

			//総在庫数
			callCreateCell(row, colIdx++).setCellValue(dto.getTotalStockNum());

			//仮在庫数
			callCreateCell(row, colIdx++).setCellValue(dto.getTemporaryStockNum());

			//キープ数
			callCreateCell(row, colIdx++).setCellValue(dto.getTotalStockNum() - dto.getTemporaryStockNum());

			//発注アラート
			callCreateCell(row, colIdx++).setCellValue(dto.getOrderAlertNum());

			//(第一倉庫)倉庫名
			callCreateCell(row, colIdx++).setCellValue(castRichTextString(dto.getWarehouseNm()));

			//(第一倉庫)在庫数
			callCreateCell(row, colIdx++).setCellValue(dto.getStockNum());

			//(第一倉庫)ロケーションNO
			callCreateCell(row, colIdx++).setCellValue(castRichTextString(dto.getLocationNo()));

			//仕様メモ
			callCreateCell(row, colIdx++).setCellValue(castRichTextString(dto.getSpecMemo()));
	 ...
	 * @throws DaoException
	 */
	public HSSFWorkbook getExportItemInfoList(SearchItemDTO searchItemDTO, HSSFWorkbook workBook, String authInfo, int sheetNo) throws DaoException {
...
//		ExtendMstItemDTO mstItemDTO = new ExtendMstItemDTO();
		...
		for (ResultItemSearchDTO dto: mstItemList) {
			...
			if (authInfo.equals(WebConst.AUTH_INFO_OK)) {
				//工場品番
				callCreateCell(row, colIdx++).setCellValue(castRichTextString(dto.getFactoryItemCode()));
			}
			//旧品番
			callCreateCell(row, colIdx++).setCellValue(castRichTextString(dto.getOldItemCode()));
			//海外閲覧権限が有るユーザーのみ見れる
			if (authInfo.equals(WebConst.AUTH_INFO_OK)) {
				//仕入国(名)
				callCreateCell(row, colIdx++).setCellValue(castRichTextString(dto.getCountry()));
				//仕入金額
...
//				callCreateCell(row, colIdx++).setCellValue(purchaceCost / 100);
				callCreateCell(row, colIdx++).setCellValue(String.valueOf(dto.getPurchaceCost()));
				//通貨
				callCreateCell(row, colIdx++).setCellValue(castRichTextString(dto.getCurrencyNm()));
			}
			//商品名
			callCreateCell(row, colIdx++).setCellValue(castRichTextString(dto.getItemNm()));
			//海外閲覧権限が有るユーザーのみ見れる
			if (authInfo.equals(WebConst.AUTH_INFO_OK)) {
				//海外商品名
				callCreateCell(row, colIdx++).setCellValue(castRichTextString(dto.getForeignItemNm()));
				//仕入先ID
				if (dto.getSysSupplierId() == 0) {
					callCreateCell(row, colIdx++).setCellValue("");
				} else {
					callCreateCell(row, colIdx++).setCellValue(dto.getSysSupplierId());
				}

			}
			//車種
			callCreateCell(row, colIdx++).setCellValue(castRichTextString(dto.getCarModel()));
			//型式
			callCreateCell(row, colIdx++).setCellValue(castRichTextString(dto.getModel()));
			//メーカー
			...
			callCreateCell(row, colIdx++).setCellValue(castRichTextString(leadTime));
			//梱包サイズ
			callCreateCell(row, colIdx++).setCellValue(castRichTextString(dto.getPackingSize()));
			//重量
			callCreateCell(row, colIdx++).setCellValue(dto.getWeight());
			//総在庫数
			callCreateCell(row, colIdx++).setCellValue(dto.getTotalStockNum());
			//仮在庫数
			callCreateCell(row, colIdx++).setCellValue(dto.getTemporaryStockNum());
			//最少ロット数
			callCreateCell(row, colIdx++).setCellValue(dto.getMinimumOrderQuantity());
			//海外閲覧権限が有るユーザーのみ見れる
			if (authInfo.equals(WebConst.AUTH_INFO_OK)) {
				//発注数
				callCreateCell(row, colIdx++).setCellValue(dto.getSumNotInStock());
			}
	 ...
	 * @throws DaoException
	 */
	public HSSFWorkbook getExportStockItemInfoList(List<SysItemIdDTO> itemIdList, HSSFWorkbook workBook, int sheetNo) throws DaoException {
		...
		for (ExtendWarehouseStockDTO dto: stockInfoList) {
			...
			//倉庫名
			callCreateCell(row, colIdx++).setCellValue(dto.getSysWarehouseId());
	 ...
	 * @throws DaoException
	 */
	public HSSFWorkbook getExportPriceInfoList(List<SysItemIdDTO> itemIdList, HSSFWorkbook workBook, String authInfo, int sheetNo) throws DaoException {
		...
		for (ItemCostPriceDTO dto : itemCostPriceList) {
			...
			if (authInfo.equals(WebConst.AUTH_INFO_OK)) {
				//仕入価格
				callCreateCell(row, colIdx++).setCellValue(itemDto.getPurchacePrice());
				//加算経費
				callCreateCell(row, colIdx++).setCellValue(itemDto.getLoading());
				//Kind原価
				callCreateCell(row, colIdx++).setCellValue(dto.getKindCost());
			}
			/***************************************以下原価情報**********************************************/
			// KTS
			callCreateCell(row, colIdx++).setCellValue(dto.getItemCost1());
			// 車楽院
			callCreateCell(row, colIdx++).setCellValue(dto.getItemCost2());
			// T-four
			callCreateCell(row, colIdx++).setCellValue(dto.getItemCost3());
			// ラルグスリテール
			callCreateCell(row, colIdx++).setCellValue(dto.getItemCost4());
			// BCR
			callCreateCell(row, colIdx++).setCellValue(dto.getItemCost5());
			// サイバーエコ
			callCreateCell(row, colIdx++).setCellValue(dto.getItemCost6());
			// KTS法人
			callCreateCell(row, colIdx++).setCellValue(dto.getItemCost7());
			// ラルグス法人
			callCreateCell(row, colIdx++).setCellValue(dto.getItemCost8());
			// KTS掛け業販
			callCreateCell(row, colIdx++).setCellValue(dto.getItemCost10());
			// Kind
			callCreateCell(row, colIdx++).setCellValue(dto.getItemCost11());
			// Brembo
			...
			/****************************************以下売価情報**********************************************/
			// KTS
			callCreateCell(row, colIdx++).setCellValue(dto.getItemPrice1());
			// 車楽院
			callCreateCell(row, colIdx++).setCellValue(dto.getItemPrice2());
			// T-four
			callCreateCell(row, colIdx++).setCellValue(dto.getItemPrice3());
			// ラルグスリテール
			callCreateCell(row, colIdx++).setCellValue(dto.getItemPrice4());
			// BCR
			callCreateCell(row, colIdx++).setCellValue(dto.getItemPrice5());
			// サイバーエコ
			callCreateCell(row, colIdx++).setCellValue(dto.getItemPrice6());
			// KTS法人
			callCreateCell(row, colIdx++).setCellValue(dto.getItemPrice7());
			// ラルグス法人
			callCreateCell(row, colIdx++).setCellValue(dto.getItemPrice8());
			// KTS掛け業販
			callCreateCell(row, colIdx++).setCellValue(dto.getItemPrice10());
			// Kind
			callCreateCell(row, colIdx++).setCellValue(dto.getItemPrice11());
			// Brembo
